...
SPOTS: dict[str, Spot] = {
    ...
    # --- 뉴캐슬 남쪽 (북쪽 기준점) ---
    # 좌표는 레드헤드 마을/헤드랜드 기준(33°00'36"S 151°43'23"E). 실제 진입하는
    # 갯바위 자리와 수백 m 차이가 날 수 있다.
    "redhead": Spot(
        "redhead", "레드헤드 (레드헤드 블러프)", -33.0100, 151.7231,
        exposed_from=(20, 190),
        note="외해 정면. 가려주는 지형이 없어 남너울도 그대로 들어온다",
    ),
...
# 주간 브리핑에서 비교할 두 기준점. 남·북 어느 쪽이 나은지 고르기 위한 것이라
# 순서(남 → 북)를 유지한다. 두 곳은 200km 떨어져 있어 같은 저기압에도 파고가
# 꽤 다르게 나온다.
WEEKLY_SPOTS: list[tuple[str, str]] = [
    ("남쪽", "little_beecroft"),
    ("북쪽", "redhead"),
]
