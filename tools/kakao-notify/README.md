...
#### 브라우저를 못 여는 환경이면 (폰·SSH·컨테이너)

기본 모드는 `localhost:5000`에 콜백 서버를 띄우고 브라우저를 엽니다. 그게
불가능하면 수동 모드를 쓰세요. 인가 코드를 손으로 옮기는 것만 다르고 결과는
같습니다.

```bash
python3 kakao_auth.py --manual
```

1. 화면에 나온 인증 주소를 브라우저에서 엽니다 (폰이면 길게 눌러 복사)
2. 카카오 로그인 후 동의하면 **접속 실패 화면**이 뜹니다. 정상입니다 —
   `localhost:5000`에는 아무것도 없으니까요
3. **그 화면의 주소창을 통째로 복사**해 붙여넣습니다

주소 전체(`http://localhost:5000/oauth?code=...&state=...`)를 붙여넣어도 되고
`code=` 뒤의 값만 붙여넣어도 됩니다.
...
가능합니다. **순서가 중요합니다** — 네트워크를 먼저 열어야 인증이 됩니다.

| | 하는 일 | 어디서 |
|---|---|---|
| 1 | 카카오 앱 만들기, REST API 키 복사 | 폰 브라우저 |
| 2 | Network access를 `Custom`으로, 카카오 도메인 2개 추가 | 폰 브라우저 |
| 3 | `kakao_auth.py --manual` 실행 | Claude Code 세션 |
| 4 | 나온 토큰을 Environment variables에 등록 | 폰 브라우저 |

2단계에서 최소한 이 둘은 열어야 3단계가 됩니다.
...
3단계는 이 저장소가 이미 복제된 클라우드 세션에서 실행하면 됩니다. 파이썬과
`requests`가 이미 깔려 있어서 폰에 아무것도 설치할 필요가 없습니다. 인증 주소가
출력되면 폰 브라우저에서 열고, 실패 화면의 주소를 복사해 세션에 붙여넣습니다.

> **다만 이렇게 하면 리프레시 토큰이 세션 대화 기록에 남습니다.** 그게 걸리시면
> 안드로이드에서 [Termux](https://f-droid.org/packages/com.termux/)를 설치해
> (`pkg install python && pip install requests`) 폰 안에서 직접 돌리세요.
> 이때는 기본 모드(`python3 kakao_auth.py`)도 그대로 됩니다 — Termux가 띄운
> localhost에 폰 브라우저가 접속할 수 있기 때문입니다.

**호버 UI 문제.** 아래 등록 화면은 마우스를 올려야 톱니바퀴가 나오는 구조라
터치에서 안 잡힐 수 있습니다. 그럴 땐 크롬 메뉴에서 **데스크톱 사이트**를
켜면 됩니다.
